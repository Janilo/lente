  ...
  const runTest = useMutation({
    mutationFn: async () => {
      ...
      try {
        ...
        toast.success(
          `Pipeline OK: ${result.insight_count} insights e ${result.recommendation_count} recomendações.`,
          { id: t },
        );
      } catch (e) {
            ...
            {data?.studies.map((s) => (
              <li key={s.id}>
                <Link
                  to="/studies/$id"
                  params={{ id: s.id }}
                  className="flex items-center justify-between gap-4 px-6 py-4 hover:bg-accent/40"
                >
                  <div className="min-w-0">
                    <h2 className="truncate text-base font-medium">{s.title}</h2>
                    {s.business_goal && (
                      <p className="truncate text-sm text-muted-foreground">{s.business_goal}</p>
                    )}
                  </div>
