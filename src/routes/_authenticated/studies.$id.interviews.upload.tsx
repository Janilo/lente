...
import { createUploadedInterview, processUploadedInterview } from "@/lib/interview-upload.functions";
...
function UploadInterview() {
 ...
 e.preventDefault();
 if (!file) { toast.error("Selecione um vídeo."); return; }
 if (!form.full_name.trim()) { toast.error("Informe o nome do respondente."); return; }
 ...
 navigate({ to: "/studies/$id/interviews/$interviewId", params: { id, interviewId: interview_id } });
 } catch (e) {
 ...
 <div>
 <Link to="/studies/$id/interviews"params={{ id }} className="text-sm text-muted-foreground hover:text-foreground">← Entrevistas do estudo</Link>
 <h1 className="mt-3 text-3xl">Enviar entrevista completa</h1>
 <p className="text-sm text-muted-foreground mt-1">
 Suba um vídeo único da entrevista. A IA transcreve e organiza as respostas conforme o roteiro do estudo.
 </p>
 </div>
 ...
 <select value={form.age_range} onChange={(e) => setForm({ ...form, age_range: e.target.value })} className="input">
 <option value="">—</option>
 {AGE_RANGES.map((r) => <option key={r} value={r}>{r}</option>)}
 </select>
 </Field>
 <Field label="Cargo / ocupação">
 <input value={form.occupation} onChange={(e) => setForm({ ...form, occupation: e.target.value })} className="input"/>
 </Field>
 ...
 <input type="file"accept="video/mp4,video/webm,video/quicktime,video/x-matroska,.mp4,.webm,.mov,.m4v,.mkv"
