...
function RespondentDetailPage() {
  ...
  if (loading || q.isLoading) {
    return (
      <div className="mx-auto max-w-5xl px-6 py-12 text-sm text-muted-foreground">Carregando…</div>
    );
  }
    ...
    return (
      <div className="mx-auto max-w-5xl px-6 py-12">
        ...
        <p className="mt-4 text-sm text-destructive">
          {(q.error as any)?.message ?? "Erro ao carregar ficha."}
        </p>
      </div>
    );
  }
    ...
    <div className="mx-auto max-w-5xl px-6 py-10 space-y-8">
      <div>
        ...
        <p className="mt-1 text-sm text-muted-foreground">
          {[p.email, p.phone, p.occupation, p.company, [p.city, p.state].filter(Boolean).join("/")]
            .filter(Boolean)
            .join("· ") || "—"}
        </p>
      </div>
