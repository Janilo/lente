      ...
      <div className="flex flex-wrap items-end justify-between gap-3">
        <div>
          ...
          <p className="mt-1 text-sm text-muted-foreground">
            Painel restrito ao administrador da plataforma.
          </p>
        </div>
        <div className="flex gap-2">
          <Link
            to="/admin/compensacao"
    ...
    <div className="py-6 space-y-4">
      <div className="grid grid-cols-2 gap-3 md:grid-cols-4">
        ...
        <FilterInput
          ...
          value={filters.research_interest}
          onChange={(v) => update("research_interest", v)}
        />
      </div>
      <div className="flex gap-2">
        <button
...
function SettingsTab() {
  ...
  useEffect(() => {
    if (data?.stt_provider) setProvider(data.stt_provider);
  }, [data?.stt_provider]);
  ...
  return (
    <div className="py-6 max-w-md space-y-4">
      ...
      <p className="text-xs text-muted-foreground">
        A mudança entra em vigor em até 60 segundos (cache do servidor).
      </p>
      <div>
...
function TagsTab() {
  ...
  const createMut = useMutation({
    mutationFn: (vars: { dimension_id: string; slug: string; label: string }) =>
      create({ data: vars }),
    onSuccess: () => {
  ...
  return (
    <div className="py-6 space-y-8">
      <p className="text-sm text-muted-foreground">
        As 5 dimensões são fixas. Cadastre os valores que ficam disponíveis para classificar
        respondentes (ex: em <em>Setor</em>: Varejo, SaaS, Financeiro…).
      </p>
      {(data?.dimensions ?? []).map((dim) => (
        <DimensionEditor
          ...
          onDelete={(id) => {
            if (confirm("Remover este valor? Respondentes perderão essa tag."))
              deleteMut.mutate(id);
          }}
        />
      ))}
...
function DimensionEditor({
  ...
  const submit = () => {
    if (!label.trim() || !slug.trim()) {
      toast.error("Preencha slug e rótulo");
      return;
    }
...
function PoolTab() {
  ...
  const poolQ = useQuery({
    ...
    queryFn: () =>
      listPool({ data: { search: appliedSearch, tagValueIds: selectedTags, onlyActive: true } }),
  });
      ...
      <div className="flex gap-2 items-end">
        <div className="flex-1">
          <label className="text-xs text-muted-foreground">
            Buscar (nome, e-mail, cargo, empresa)
          </label>
          <input
            value={search}
      ...
      <div className="space-y-2">
        ...
        {allValues.length === 0 && (
          <p className="text-xs text-muted-foreground italic">
            Cadastre valores de tag em "Tags"para filtrar o pool.
          </p>
        )}
      </div>
    ...
    <div className="rounded-sm border border-border bg-card p-4 space-y-3">
      <div className="flex flex-wrap items-baseline justify-between gap-2">
        ...
        <div className="flex items-center gap-4 text-xs text-muted-foreground">
          <span>
            <strong className="text-foreground">{respondent.studies_count}</strong> estudos
          </span>
          <span>
            <strong className="text-foreground">{respondent.completed_count}</strong> concluídas
          </span>
          {respondent.avg_quality_score != null && (
            <span>
              Qualidade{" "}
              <strong className="text-foreground">
                {Number(respondent.avg_quality_score).toFixed(1)}
              </strong>
            </span>
          )}
          {respondent.last_participation_at && (
            <span>
              Última: {new Date(respondent.last_participation_at).toLocaleDateString("pt-BR")}
            </span>
          )}
          <Link
            to="/admin/respondentes/$id"
      ...
      <div className="flex flex-wrap gap-1.5 items-center">
        {respondent.tags.map((t) => (
          <span
            key={t.tag_value_id}
            className="inline-flex items-center gap-1.5 bg-secondary px-2.5 py-1 text-xs"
          >
            <span className="text-muted-foreground">{t.dimension}:</span>
            <span>{t.label}</span>
            <button
              onClick={() => onUnassign(t.tag_value_id)}
              className="text-muted-foreground hover:text-destructive"
            >
              ×
            </button>
