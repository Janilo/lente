    ...
    <div className="mx-auto max-w-4xl px-6 py-12 space-y-6">
      <div>
        ...
        <p className="text-sm text-muted-foreground mt-1">
          {data.interview.status === "completed" ? "Concluída" : "Em andamento"} · Iniciada em{" "}
          {new Date(data.interview.started_at).toLocaleString("pt-BR")}
        </p>
      </div>
            ...
            <li key={a.id} className="rounded-sm border border-border bg-card p-5 space-y-3">
              ...
              {a.status === "failed" && (
                <div className="text-xs text-destructive">
                  Falha: {a.error_message ?? "erro desconhecido"}
                </div>
              )}
...
function StatusPill({ status }: { status: string }) {
  ...
  return (
    <span
      className={`inline-flex items-center rounded-sm px-2 py-0.5 text-xs font-semibold ${m.cls}`}
    >
      {m.label}
    </span>
  );
