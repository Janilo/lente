...
function StudyEditor() {
  ...
  const editQ = useMutation({
    mutationFn: async (q: { id: string; position: number; text: string; intent: string }) =>
      upsertQ({
        data: { id: q.id, study_id: id, position: q.position, text: q.text, intent: q.intent },
      }),
    onSuccess: () => qc.invalidateQueries({ queryKey: ["study", id] }),
    onError: (e: Error) => toast.error(e.message),
  });
  ...
  // All hooks are declared above this point (rules-of-hooks): only branch to a
  // child route / loading / empty states after every hook has run.
  ...
  if (!data?.study)
    return <div className="mx-auto max-w-4xl px-6 py-12">Estudo não encontrado.</div>;
    ...
    <div className="mx-auto max-w-4xl px-6 py-12 space-y-10">
      ...
      <section className="rounded-sm border border-border bg-card p-6 space-y-4">
        ...
        <Field
          ...
          hint="O que você quer aprender ou decidir com esta pesquisa."
        >
          <textarea
            rows={3}
        ...
        <Field
          label="Contexto da marca / produto"
          hint="O que o entrevistado e a IA precisam saber para interpretar bem as respostas."
        >
          <textarea
            rows={4}
            value={form.context}
        ...
        <div className="grid gap-4 md:grid-cols-2 md:items-start">
          <Field
            label="Profundidade dos follow-ups"
            hint="Quantas perguntas de aprofundamento a IA pode fazer por pergunta (0–5)."
          >
            <input
              type="number"
              min={0}
              ...
              onChange={(e) =>
                setForm({
                  ...form,
                  max_followups: Math.max(0, Math.min(5, Number(e.target.value))),
                })
              }
              className="w-full rounded-md border border-input bg-background px-3 py-2 text-sm"
            />
          ...
          <Field
            ...
            hint={
              canPublish
                ? "Controla se o link da entrevista está ativo para os respondentes."
                : "Publicação ainda não liberada para sua conta. Solicite ao administrador."
            }
          >
            <select
              value={form.status}
            ...
            >
              ...
              <option value="published" disabled={!canPublish && form.status !== "published"}>
                Publicado (link ativo)
                {!canPublish && form.status !== "published" ? "— bloqueado" : ""}
              </option>
      ...
      <section className="rounded-sm border border-border bg-card p-6 space-y-4">
        ...
        {data.questions.length === 0 ? (
          <p className="text-sm text-muted-foreground">
            Adicione perguntas para guiar a entrevista. A IA poderá complementar com follow-ups.
          </p>
        ) : (
            ...
            {data.questions.map((q, idx) => (
              <li key={q.id} className="rounded-md border border-border bg-background p-4">
                <div className="flex items-center justify-between text-xs text-muted-foreground">
                  ...
                  <button onClick={() => removeQ.mutate(q.id)} className="hover:text-destructive">
                    Remover
                  </button>
                </div>
                <textarea
                ...
                <input
                  ...
                  placeholder="Intenção (opcional) — o que esta pergunta deve revelar"
                  onBlur={(e) =>
                    e.target.value !== (q.intent ?? "") &&
                    editQ.mutate({
                      id: q.id,
                      position: q.position,
                      text: q.text,
      ...
      <section className="rounded-sm border border-border bg-card p-6">
        ...
        <p className="mt-4 text-xs text-muted-foreground">
          No link web, o respondente cria conta e grava em vídeo. No Telegram, ele responde por
          texto, áudio ou vídeo direto no chat — sem precisar de conta.
        </p>
      </section>
    </div>
  );
