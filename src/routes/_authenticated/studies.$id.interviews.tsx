...
type Insights =
  | (Omit<NonNullable<RawRow["insights"]>, "answer_summaries"> & {
...
function InterviewsTable() {
  ...
  if (isLoading)
    return (
      <div className="mx-auto max-w-7xl px-6 py-12 text-sm text-muted-foreground">Carregando…</div>
    );
  ...
  return (
    <div className="mx-auto max-w-[1600px] px-6 py-12 space-y-6">
      <div className="flex items-start justify-between gap-4 flex-wrap">
        ...
        <Link
          to="/studies/$id/interviews/upload"
          params={{ id }}
          ...
          <table className="w-full text-sm border-collapse">
            <thead className="bg-muted/40">
              ...
              <tr className="bg-background/60 border-t border-border">
                ...
                <FilterCell
                  ...
                  onChange={(v) => setFilter("started_at", v)}
                />
                <FilterCell />
                <FilterCell
                ...
                <FilterCell
                  ...
                  onChange={(v) => setFilter("respondent", v)}
            ...
            <tbody>
              {filteredRows.length === 0 ? (
                <tr>
                  <td
                    colSpan={10 + data.questions.length}
...
function Th({ label, w }: { label: string; w?: string }) {
  return (
    <th
      className={`px-3 py-2.5 text-xs font-medium text-muted-foreground border-b border-border ${w ?? ""}`}
    >
      {label}
    </th>
  );
...
function RowItem({
  ...
  const answerSummary = (qid: string) =>
    insights?.answer_summaries?.find((s) => s.question_id === qid)?.summary ?? "—";
  const respondentLabel = [
    row.respondent_name,
    [row.respondent_city, row.respondent_state].filter(Boolean).join("/"),
  ]
    .filter(Boolean)
    .join("· ");
    ...
    <tr className="border-b border-border hover:bg-muted/20">
      <td className="px-3 py-3 align-top">
        <Link
          ...
          params={{ id: studyId, interviewId: row.id }}
          className="text-primary hover:underline font-medium"
        >
          #{row.sequence}
        </Link>
        {row.source === "upload" && (
          <div className="text-[10px] text-muted-foreground mt-0.5">upload</div>
        )}
      </td>
      ...
      <td className="px-3 py-3 align-top">
        {qualityLabel(row.avg_quality_score, insights?.quality ?? null)}
      </td>
      ...
      <td className="px-3 py-3 align-top">
        {insights?.bullet_summary?.length ? (
          <ul className="space-y-0.5 text-xs leading-relaxed">
            {insights.bullet_summary.map((b, i) => (
              <li key={i}>– {b}</li>
            ))}
          </ul>
        ) : (
...
function rowFieldText(
  ...
  switch (key) {
    case "started_at":
      return new Date(row.started_at).toLocaleString("pt-BR");
    case "status":
      return STATUS_LABEL[row.status] ?? row.status;
    case "quality":
      return qualityLabel(row.avg_quality_score, insights?.quality ?? null);
    case "respondent":
      return [row.respondent_name, row.respondent_city, row.respondent_state]
        .filter(Boolean)
        .join("");
    case "segments":
      return (insights?.segments ?? []).join("");
    case "bullets":
      return (insights?.bullet_summary ?? []).join("");
    case "tagline":
      return insights?.tagline ?? "";
    case "tags":
      return (insights?.tags ?? []).join("");
  }
  if (key.startsWith("q:")) {
