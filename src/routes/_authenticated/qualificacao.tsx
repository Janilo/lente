...
function QualificationPage() {
 ...
 <h2 className="text-sm font-semibold uppercase tracking-wider text-muted-foreground">Dados básicos</h2>
 <Field label="Nome completo *">
 <input required maxLength={120} value={fullName} onChange={(e) => setFullName(e.target.value)} className={inputCls} />
 </Field>
 <div className="grid gap-3 sm:grid-cols-2">
 <Field label="Empresa">
 <input maxLength={120} value={company} onChange={(e) => setCompany(e.target.value)} className={inputCls} />
 </Field>
 <Field label="Cargo / função">
 <input maxLength={120} value={occupation} onChange={(e) => setOccupation(e.target.value)} className={inputCls} />
 </Field>
 <Field label="Cidade">
 <input maxLength={80} value={city} onChange={(e) => setCity(e.target.value)} className={inputCls} />
 </Field>
 <Field label="Estado (UF)">
 <input maxLength={40} value={state} onChange={(e) => setState(e.target.value)} className={inputCls} />
 </Field>
 ...
 <input type="url"maxLength={300} placeholder="https://linkedin.com/in/…"value={linkedin} onChange={(e) => setLinkedin(e.target.value)} className={inputCls} />
 </Field>
 </div>
 </section>

 <section className="space-y-3">
 <h2 className="text-sm font-semibold uppercase tracking-wider text-muted-foreground">Segmentação</h2>
 <p className="text-xs text-muted-foreground">Escolha o que mais se encaixa. Opcional, mas ajuda na curadoria de estudos.</p>
 ...
 >
 <option value="">{dim.values.length === 0 ? "Sem opções cadastradas": "— selecionar —"}</option>
 {dim.values.map((v) => (
