    ...
    <div className="mx-auto max-w-3xl px-6 py-12 space-y-6">
      <div>
        ...
        <p className="mt-2 text-sm text-muted-foreground">
          Aqui você acessa e controla os dados que você forneceu como entrevistado. Conforme a LGPD,
          você pode baixar seus dados ou solicitar exclusão a qualquer momento.
        </p>
      </div>
              ...
              <div className="flex items-start justify-between gap-3 flex-wrap">
                <div>
                  ...
                  <div className="text-xs text-muted-foreground mt-1">
                    {iv.status === "completed" ? "Concluída" : "Em andamento"} · iniciada em{" "}
                    {new Date(iv.started_at).toLocaleString("pt-BR")}
                  </div>
                </div>
                <div className="flex gap-2">
                  <button
                    onClick={() => handleExport(iv.id)}
                  ...
                  <button
                    onClick={() => {
                      if (
                        confirm(
                          "Apagar permanentemente todos os seus dados desta entrevista? Esta ação não pode ser desfeita.",
                        )
                      )
                        del.mutate(iv.id);
                    }}
