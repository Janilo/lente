...
function RespondentsPanel() {
  ...
  if (isLoading)
    return (
      <div className="mx-auto max-w-6xl px-6 py-12 text-sm text-muted-foreground">Carregando…</div>
    );
  if (!data) return null;
    ...
    <div className="mx-auto max-w-6xl px-6 py-12 space-y-6">
      <div>
        ...
        <p className="mt-1 text-sm text-muted-foreground">
          {data.respondents.length} {data.respondents.length === 1 ? "respondente" : "respondentes"}{" "}
          · {data.study.total_questions} pergunta(s) no questionário
        </p>
      </div>
      ...
      <div className="rounded-sm border border-[color:var(--lente-amber)]/40 bg-[color:var(--lente-amber-soft)] dark:bg-[color:var(--lente-amber)]/10 p-4 text-xs text-muted-foreground">
        <strong className="text-foreground">Aviso LGPD:</strong> os dados pessoais abaixo (nome,
        email, gravações, transcrições) só podem ser usados para a finalidade declarada no termo
        aceito pelo respondente. Não os compartilhe sem base legal. Você pode atender pedidos de
        exclusão pelo botão "Apagar dados".
      </div>
                ...
                <tr key={r.interview_id} className="border-t border-border align-top">
                  ...
                  <td className="px-4 py-3">
                    ...
                    {r.finished_at && (
                      <div className="text-xs text-muted-foreground">
                        Fim: {new Date(r.finished_at).toLocaleString("pt-BR")}
                      </div>
                    )}
                  </td>
                  ...
                  <td className="px-4 py-3">
                    {r.avg_quality === null ? (
                      ...
                      <span
                        className={`text-sm font-medium ${r.avg_quality >= 70 ? "text-[color:var(--lente-success)]" : r.avg_quality >= 40 ? "text-[color:var(--lente-amber)]" : "text-destructive"}`}
                      >
                        {r.avg_quality}/100
                      </span>
                    )}
                  ...
                  <td className="px-4 py-3 text-right">
                    <div className="inline-flex flex-col gap-1 items-end">
                      <button
                        onClick={() => setOpenInterview(r.interview_id)}
                        className="text-xs text-primary hover:underline"
                      >
                        Ver detalhes
                      </button>
                      <button
                        onClick={() => handleExport(r.interview_id, r.full_name || "respondente")}
                      ...
                      <button
                        onClick={() => {
                          if (
                            confirm(
                              "Apagar permanentemente todos os dados deste respondente (vídeos, transcrições, entrevista)?",
                            )
                          )
                            del.mutate(r.interview_id);
                        }}
...
function RespondentDetailDrawer({
  interviewId,
  onClose,
}: {
  interviewId: string;
  ...
  return (
    ...
    >
      <div
        className="mx-auto max-w-3xl my-12 rounded-sm border border-border bg-card border border-border"
          ...
          {isLoading || !data ? (
            ...
            <>
              <div className="text-sm">
                ...
                <div>
                  Status: {data.interview.status === "completed" ? "Concluída" : "Em andamento"}
                </div>
              </div>
                  ...
                  <li key={a.id} className="rounded-md border border-border p-4">
                    ...
                    {a.video_url ? (
                      <video
                        src={a.video_url}
                        controls
                        className="mt-3 w-full rounded-md bg-black aspect-video"
                      />
                    ) : (
                    ...
                    <p className="mt-1 whitespace-pre-wrap text-sm">
                      {a.transcript || (
                        <span className="text-muted-foreground italic">
                          Sem transcrição (status: {a.status})
                        </span>
                      )}
                    </p>
