  ...
  head: () => ({
    meta: [
      { title: "Convite para entrevista — Lente" },
      {
        name: "description",
        content:
          "Você foi convidado a participar de uma entrevista em vídeo. Confira o consentimento e prepare-se para gravar suas respostas.",
      },
      { property: "og:title", content: "Convite para entrevista — Lente" },
      {
        property: "og:description",
        content: "Participe de uma entrevista em vídeo conduzida com a Lente.",
      },
...
function PublicStudyPage({ slug }: { slug: string }) {
  ...
  if (error)
    return (
      <div className="mx-auto max-w-2xl px-6 py-20 text-sm text-destructive">
  ...
  return (
    <div className="mx-auto max-w-2xl px-6 py-16">
      <p className="text-xs uppercase tracking-widest text-muted-foreground">
        Você foi convidado para uma entrevista
      </p>
      <h1 className="mt-3 text-4xl font-semibold leading-tight">{study.title}</h1>
      {study.context && (
        <p className="mt-6 whitespace-pre-wrap text-base text-muted-foreground">{study.context}</p>
      )}
      {study.target_audience && (
        <p className="mt-4 text-sm text-muted-foreground">
          <strong className="text-foreground">Público:</strong> {study.target_audience}
        </p>
      )}
      <p className="mt-6 text-sm text-muted-foreground">
        {questionCount} pergunta(s). Suas respostas serão gravadas em vídeo.
      </p>
        ...
        {loading ? (
          ...
          <div className="flex flex-col gap-3">
            <p className="text-sm text-muted-foreground">
              Crie uma conta ou entre para participar.
            </p>
