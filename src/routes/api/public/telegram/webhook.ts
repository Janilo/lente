...
async function createInterviewAndStart(chat_id: number, session: any) {
  ...
  if (error || !created) {
    await sendMessage(
      chat_id,
      "Não consegui iniciar sua entrevista. Tente novamente em alguns minutos.",
    );
    return;
  }
  await upsertSession(chat_id, {
...
async function advance(chat_id: number, interview_id: string) {
  ...
  if (next.type === "processing") {
    await sendMessage(
      chat_id,
      "⏳ Ainda estou processando sua última resposta. Aguarde só um instante…",
    );
    return;
  }
...
async function recordAnswer(
  chat_id: number,
  session: any,
  opts: { transcript?: string; mediaBlob?: Blob; duration?: number },
) {
  ...
  if (opts.mediaBlob) {
    try {
      ...
      await supabaseAdmin.storage
        .from(BUCKET)
        .upload(path, buf, { contentType: "audio/ogg", upsert: true });
      ...
      if (cleaned.length < 2) {
        await supabaseAdmin
          .from("answers")
          .update({ status: "failed", error_message: "Nenhuma fala detectada." })
          .eq("id", ans.id);
        await sendMessage(
          chat_id,
          "Não consegui entender o áudio 😕. Pode responder novamente (texto ou áudio mais claro)?",
        );
        return;
      }
      await supabaseAdmin
        .from("answers")
        .update({ status: "ready", transcript: cleaned })
        .eq("id", ans.id);
      try {
    ...
    try {
      await scoreAnswerInternal(ans.id, opts.transcript ?? "");
    } catch (e) {
      console.error("score failed", e);
    }
  }
...
async function handleMessage(message: TgMessage) {
  ...
  if (text.startsWith("/start")) {
    ...
    if (!payload) {
      await sendMessage(
        chat_id,
        "Olá! 👋 Para participar de uma pesquisa, abra o link compartilhado pelo pesquisador.",
      );
      return;
    }
  ...
  if (!session) {
    await sendMessage(
      chat_id,
      "Não encontrei uma pesquisa ativa para você. Use o link compartilhado pelo pesquisador para começar.",
    );
    return;
  }
  ...
  if (!session.interview_id || !session.pending_question_id) {
    await sendMessage(
      chat_id,
      "Não há pergunta pendente. Use o link compartilhado pelo pesquisador para iniciar uma nova pesquisa.",
    );
    return;
  }
