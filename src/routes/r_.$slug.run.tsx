...
export const Route = createFileRoute("/r_/$slug/run")({
  head: () => ({
    meta: [
      ...
      {
        name: "description",
        content: "Sessão de gravação da entrevista em vídeo. Responda às perguntas no seu ritmo.",
      },
...
function RunInner({ slug }: { slug: string }) {
  ...
  if (!interviewId || !step || stepLoading) {
    return (
      <div className="mx-auto max-w-2xl px-6 py-20 text-sm text-muted-foreground">Preparando…</div>
    );
  }
    ...
    return (
      <div className="mx-auto max-w-2xl px-6 py-20 text-center">
        ...
        <p className="mt-3 text-sm text-muted-foreground">
          Sua entrevista foi concluída. Você pode fechar esta página.
        </p>
        <Link
          to="/r/$slug"
          params={{ slug }}
  ...
  return (
    <div className="rounded-sm border border-border bg-card p-4">
      <div className="relative">
        ...
        />
        {state === "recording" && (
          <div className="absolute left-3 top-3 flex items-center gap-2 bg-black/70 px-3 py-1 text-xs font-medium text-white">
      ...
      <div className="mt-4 flex items-center justify-between gap-3">
        <div className="text-xs text-muted-foreground">
          {state === "idle" &&
            camState === "idle" &&
            "Conceda acesso à câmera e microfone para começar."}
