    ...
    <section className="rounded-sm border border-border bg-card p-6 space-y-4">
      <div className="flex items-center justify-between gap-3 flex-wrap">
        <div>
          ...
          <p className="mt-1 text-sm text-muted-foreground">
            Exibidas antes do consentimento. Use para filtrar respondentes ou apenas registrar
            perfil.
          </p>
        </div>
        <button
      ...
      {isLoading ? (
        ...
        <p className="text-sm text-muted-foreground">
          Nenhuma pergunta de triagem. Sem triagem, todos seguem direto para o consentimento.
        </p>
      ) : (
        <ol className="space-y-4">
          {(data!.questions as ScreenerQuestion[]).map((q, idx) => (
            <ScreenerItem
              ...
              question={q}
              onSave={(patch) =>
                upsertFn({ data: { ...q, ...patch, study_id: studyId } })
                  .then(invalidate)
