...
function PreviewDialog({
  ...
  const updateBlock = (bi: number, patch: Partial<Block>) => {
    const next = {
      ...script,
      blocks: script.blocks.map((b, i) => (i === bi ? { ...b, ...patch } : b)),
    };
    setScript(next);
  };
  ...
  const addBlock = () => {
    setScript({
      ...
      blocks: [
        ...script.blocks,
        { title: "Novo bloco", objective: "", questions: [{ text: "", intent: "" }] },
      ],
    });
  };
    ...
    <Dialog open={open} onOpenChange={(v) => !v && onClose()}>
      <DialogContent className="max-w-3xl max-h-[85vh] overflow-y-auto">
        <DialogHeader>
          ...
          <DialogDescription>
            Confira a estrutura identificada: blocos, objetivos e perguntas. Edite o que precisar
            antes de adicionar ao roteiro.
          </DialogDescription>
        </DialogHeader>
        ...
        <div className="space-y-5">
          <div>
            <label className="text-xs font-medium text-muted-foreground">
              Cabeçalho (referência — não é salvo)
            </label>
            <input
              value={script.header}
          ...
          <div className="rounded-sm border border-border bg-card p-4">
            ...
            <p className="text-xs text-muted-foreground mt-0.5">
              Notas, instruções de encerramento ou lembretes para o entrevistador. Serão salvas como
              item final do roteiro.
            </p>
            <textarea
              rows={3}
