...
function Step({ title, sub, state }: { title: string; sub?: string; state: StepState }) {
  const titleCls =
    state === "disabled"
...
export function PipelineStatus({
  ...
  // Upload step
  const uploadState: StepState = !data
    ? "pending"
    : data.answers.uploading > 0
      ? "active"
      : data.last_answer?.status === "failed"
        ? "failed"
  ...
  // Transcription step
  const transcribeState: StepState = !data
    ? "pending"
    : data.answers.transcribing > 0
      ? "active"
      : data.answers.failed > 0
        ? "failed"
        : data.answers.ready > 0
          ? "done"
          : "pending";
  ...
  const synthState: StepState =
    variant === "respondent" ? "disabled" : synthDone ? "done" : "pending";
  ...
  const fuSub =
    fu === "deciding"
      ? "Avaliando se vale aprofundar…"
      : fu === "ready"
        ? "Pergunta de aprofundamento gerada"
        : fu === "skipped"
          ? "Sem follow-up — seguindo adiante"
          : fu === "exhausted"
            ? "Limite de follow-ups atingido"
  ...
  const lastError = data?.last_answer?.status === "failed" ? data.last_answer.error_message : null;
