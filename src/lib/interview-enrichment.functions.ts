...
export async function enrichInterviewInternal(interview_id: string): Promise<void> {
  ...
  const { data: iv } = await supabaseAdmin
    .from("interviews")
    .select("id, study_id, external_respondent, respondent_id, studies:study_id(title, context, target_audience, business_goal)")
    .eq("id", interview_id)
    .maybeSingle() as {
      data: {
        id: string; study_id: string; external_respondent: Record<string, unknown> | null; respondent_id: string;
        studies: { title: string; context: string | null; target_audience: string | null; business_goal: string | null } | null;
      } | null;
    };
  if (!iv) return;
  ...
  const { data: answers } = await supabaseAdmin
    .from("answers")
    .select("id, question_id, question_text, transcript, is_followup, status, parent_answer_id, created_at")
  ...
  const qBlock = (questions ?? []).map((q, i) => `Q${i + 1} (id=${q.id}): ${q.text}${q.intent ? `\n  Intenção: ${q.intent}` : ""}`).join("\n");
  const aBlock = readyAnswers
    .map((a) => `[${a.is_followup ? "Follow-up" : "Pergunta"}] question_id=${a.question_id ?? "?"}\n  P: ${a.question_text}\n  R: ${a.transcript}`)
    .join("\n\n");
  ...
  const tools = [{
    ...
    function: {
      ...
      parameters: {
        ...
        required: ["quality", "segments", "tags", "bullet_summary", "tagline", "answer_summaries"],
        additionalProperties: false,
      },
    },
  }];
